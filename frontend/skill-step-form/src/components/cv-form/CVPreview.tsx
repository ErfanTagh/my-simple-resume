      ...
      <div className="space-y-6">
        ...
        <div className="border-b pb-4">
          ...
          {personalInfo.professionalTitle && (
            <p className="text-lg text-muted-foreground mt-1">{personalInfo.professionalTitle}</p>
          )}
              ...
              {workExperience.map((exp, index) => (
                (exp.position || exp.company) && (
                  <div key={index} className="space-y-1">
                    ...
                    {exp.technologies && exp.technologies.length > 0 && exp.technologies.some(t => t.technology) && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {exp.technologies.map((tech, i) => tech.technology && (
                  ...
                  <div key={index} className="space-y-1">
                    ...
                    {proj.technologies && proj.technologies.length > 0 && proj.technologies.some(t => t.technology) && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {proj.technologies.map((tech, i) => tech.technology && (
                          <span key={i} className="px-2 py-0.5 bg-secondary text-secondary-foreground rounded text-xs">
                            {tech.technology}
                          </span>
                        ))}
              ...
              {languages.map((lang, index) => (
                lang.language && (
                  <div key={index} className="flex items-center gap-1">
                    ...
                    {lang.proficiency && (
                      <span className="text-muted-foreground whitespace-nowrap">- {lang.proficiency}</span>
                    )}
                  </div>
                )
              ))}
        ...
        {/* Interests */}
        {personalInfo.interests && personalInfo.interests.length > 0 && personalInfo.interests.some(i => i.interest) && (
          <div>
              ...
              {personalInfo.interests.map((item, index) => (
                item.interest && (
                  <span key={index} className="text-sm text-muted-foreground">
                    {item.interest}
                    {index < personalInfo.interests.length - 1 && personalInfo.interests[index + 1]?.interest && " • "}
                  </span>
                )
              ))}
            </div>
